...
let connectionX = "";
let connection = mongo.connect(mongoURL, function (db) {
    connectionX = db;
})
...
exports.listHotels = ((message, callback) => {
    try {
        if (message === null || message === undefined || message === '') {
            ...
            console.log(capitalizeFirstLetter(searchCriteria.city.toLowerCase()));
            console.log(searchCriteria.city.toLowerCase());
            //
            ...
            mongo.collection("Hotel").find({ city: searchCriteria.city.toLowerCase()}).toArray( function (err, results) {
                console.log(err);
                console.log(results);
                    ...
                    if (hotel !== null || hotel !== "") {
                        ...
                        mysql.fetchData(function (err, result1) {
                            if (err) {
                                ...
                                console.log(result1);
                                if (result1 !== null) {
                                    if (result1[0].total < hotel.totalRooms) {
                        ...
                        }, "select count(bookingid) as total from hotelbooking where hotelbooking.hotelId = " + hotel.hotelId + " and hotelbooking.fromDate = '" + searchCriteria.checkIn_date + "' and hotelbooking.toDate = '" + searchCriteria.checkOut_date + "'")
                    }
                }, function (err) {
                    if (err) {
                        callback(err)
